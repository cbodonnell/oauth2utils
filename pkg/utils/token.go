...
func TryGetToken(ctx context.Context, oc *oauth.OIDCClient, tokenDir string) *oauth2.Token {
	token, err := persistence.LoadToken(tokenDir)
	if err == nil {
		token, _ = oc.TokenSource(ctx, token).Token()
		if token != nil {
			persistence.SaveToken(token, tokenDir)
		}
	}
	return token
}
